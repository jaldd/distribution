...
public final class SelectItemParserExt implements QueryPartProcessor {
    ...
    public SelectItemParserExt(Query query, boolean allowExpressionBasedSelectItems) {
        _query = query;
    ...
    public void parse(String delim, String itemToken) throws MetaModelException {
        ...
        if (indexOfAlias != -1) {
            ...
            // add by xinzp 2021-05-12
            // Default assumption is space being delimiter for column and
            // alias.. If otherwise use DoubleQuotes or [] around columnName
            ...
            // add by xinzp 允许添加别名的时候使用 AS 关键字
                ...
                if(tokens[tokens.length-2].equalsIgnoreCase("AS")){
                    itemToken = itemToken.substring(0,itemToken.lastIndexOf(tokens[tokens.length-2])).trim();
                }else{
                    itemToken = itemToken.substring(0,itemToken.lastIndexOf(tokens[tokens.length-1])).trim();
                }
            }else{
                throw new QueryParserException("Not capable of parsing SELECT token: " + itemToken);
            }
        ...
        if (fromItem != null) {
            if ("*".equals(columnName)) {
                ...
                final SelectItem subQuerySelectItem =
                        new SelectItemParserExt(subQuery, _allowExpressionBasedSelectItems).findSelectItem(columnName);
