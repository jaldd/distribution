...
public final class FromItemParserExt implements QueryPartProcessor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FromItemParserExt.class);

    private final Query _query;
    private final DataContext _dataContext;

    public FromItemParserExt(DataContext dataContext, Query query) {
    ...
    public void parse(String delim, String itemToken) {
        ...
        if (parenthesisStart != -1) {
            ...
            final Query subQuery = new QueryParserExt(_dataContext, subQueryString).parse();
    ...
    private FromItem parseTableItem(String itemToken) {
        ...
        if (delimiterMap.containsKey(startDelimiter)) {
            ...
            // add by xinzp 2021-05-12 允许添加别名的时候使用 AS 关键字
        ...
        if (table == null) {
            if(tableNameToken.equalsIgnoreCase("dual")){
                result = new FromItem(itemToken);
            }else{
                throw new QueryParserException("Not capable of parsing FROM token: " + itemToken);
            }
        }else{
            result = new FromItem(table);
        }
